...
func (j *LuaTable) MarshalJSONBuf(buf fflib.EncodingBuffer) error {
	...
	_ = obj
	_ = err
	/* Falling back. type=types.LuaTypeMap kind=map */
	buf.WriteString(`{"v":`)
...
handle_Value:
	...
	/* handler: j.Value type=types.LuaTypeMap kind=map quoted=false*/

	{

		{
			if tok != fflib.FFTok_left_bracket && tok != fflib.FFTok_null {
				return fs.WrapErr(fmt.Errorf("cannot unmarshal %s into Go value for LuaTypeMap", tok))
			}
		}
		...
		if tok == fflib.FFTok_null {
			...
			j.Value = make(map[LuaTypeType]LuaType, 0)
			...
			for {
				...
				/* handler: k type=types.LuaTypeType kind=int8 quoted=false*/

				{
					if tok != fflib.FFTok_integer && tok != fflib.FFTok_null {
						return fs.WrapErr(fmt.Errorf("cannot unmarshal %s into Go value for LuaTypeType", tok))
					}
				}
				...
				tok = fs.Scan()
				/* handler: tmpJValue type=types.LuaType kind=interface quoted=false*/

				{
					/* Falling back. type=types.LuaType kind=interface */
					tbuf, err := fs.CaptureField(tok)
					if err != nil {
						return fs.WrapErr(err)
					}

					err = json.Unmarshal(tbuf, &tmpJValue)
					if err != nil {
						return fs.WrapErr(err)
					}
				}
